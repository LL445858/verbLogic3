# -*- coding: utf-8 -*-
"""
strategic_multiplication_lift.py

Implements the user's new definition of "战略科学家倍增指数 (Lift)" and produces:
- Lift(Δ) curves (pooled / case-aggregated / scientist-aggregated)
- CSV outputs for reproducibility
- Matplotlib visualizations

Inputs (default: same directory as this script, or --input_dir):
- v.txt : JSON dict {"data1":[verb1, verb2, ...], ...}
- a.txt : JSON-like text with possible duplicate verb keys; contains per-verb attributes incl. "执行者"
- c.txt : JSON-like text with possible duplicate verb keys; contains per-verb stage label
- 执行者统计_角色标注.xlsx : mapping table with columns including "执行者" and "执行者类别" (0/1/2/3)

New definitions (as specified by user):
- Event sequence includes only verbs with an explicit "执行者" attribute.
- Trigger:
    T_{d,t} = (r_{d,t} = 3) AND (s_{d,t} ∈ {知识协作, 知识整合, 知识规划})
- Outcome within window Δ:
    Y_{d,t}(Δ) = 1 if ∃k∈{1..Δ} such that (r_{d,t+k} > 1) AND (s_{d,t+k} = 知识重构)
- Baseline:
    B_{d,t} = (r_{d,t} != 3) AND (s_{d,t} ∈ {知识协作, 知识整合, 知识规划})
- Lift(Δ) = E[Y(Δ) | T] / E[Y(Δ) | B]

Aggregation:
- Pooled (event-weighted): compute E[·|T] and E[·|B] over all events.
- Case aggregation: compute per-case p_T(d), p_B(d), then aggregate lifts across cases.
- Scientist aggregation: for each strategic scientist s (role=3), numerator uses that scientist's T events;
  denominator uses baseline B events within the same set of cases where s appears (context-matched).

Usage:
    python strategic_multiplication_lift.py --input_dir /path/to/files --max_delta 12

Outputs (in --output_dir, default same as input_dir):
    lift_by_delta.csv
    case_lift_by_delta.csv
    scientist_lift_by_delta.csv
    lift_curves.png
"""
